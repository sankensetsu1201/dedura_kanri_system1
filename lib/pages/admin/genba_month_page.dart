// lib/pages/admin/genba_month_page.dart
import 'package:flutter/material.dart';
import 'package:cloud_firestore/cloud_firestore.dart';
import 'package:intl/intl.dart';

class GenbaMonthPage extends StatefulWidget {
  const GenbaMonthPage({super.key});

  @override
  State<GenbaMonthPage> createState() => _GenbaMonthPageState();
}

class _GenbaMonthPageState extends State<GenbaMonthPage> {
  ...
  Widget _buildSummary(String monthPrefix) {
    final Map<String, double> siteDays = {};
    final Map<String, double> siteOvertime = {};
    ...
    for (final doc in _demenDocs) {
      ...
      siteDays[genbaName] = (siteDays[genbaName] ?? 0) + dayCount;

      // 残業
      final overtime = (data["overtime"] ?? 0).toDouble();
      siteOvertime[genbaName] = (siteOvertime[genbaName] ?? 0) + overtime;
      ...
      siteExpenses[genbaName] = (siteExpenses[genbaName] ?? 0) + totalExp;
    }
    ...
    return ListView(
      ...
      children: siteDays.entries.map((entry) {
        final site = entry.key;
        final days = entry.value;
        final overtime = siteOvertime[site] ?? 0;
        final expenses = siteExpenses[site] ?? 0;
